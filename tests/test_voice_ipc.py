...
class FakeBackend:
    ...
    def requirements(self) -> dict[str, object]:
        return {"voice": {"available": True, "details": "ready"}}
...
class VoiceIPCTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        provider = FileCredentialProvider(Path(self.temporary.name) / "runtime" / "ipc.credential")
    ...
    def request(self, operation: str, payload=None, *, session="s1", credential="valid"):
    ...
    def test_voice_start_and_events_use_authenticated_identity(self) -> None:
        ...
        self.assertFalse(started.result["wake"]["active"])
    ...
    def test_transcript_uses_authenticated_shared_assistant_admission(self) -> None:
        ...
        reply = next(event for event in reversed(events) if event["kind"] == "reply")
        self.assertEqual(reply["text"], "reply: hello jl")
    ...
    def test_wake_operations_are_rejected(self) -> None:
        for operation, payload in (
            ("wake-start", {}),
            ("wake-stop", {}),
            ("wake-test-start", {"phrase": "hello jl"}),
            ("wake-phrase-set", {"phrase": "hello jl"}),
        ):
            response = self.request(operation, payload)
            self.assertFalse(response.ok)
            self.assertEqual(response.error_code, "wake_disabled")
